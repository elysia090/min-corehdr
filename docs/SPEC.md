...
Compile-complete also includes:

* emitted record types carry preserve_access_index via file-level #pragma clang attribute push/pop
* this is required because CO-RE relocations encode access indices that must survive compilation
...
10.2 Local-vs-Kernel Distinction

Object-local BTF contains both kernel-derived types and program-local types, such as event structs, map value types, and internal helper types. The tool must distinguish them by attempting to resolve each candidate type against the base BTF.

Resolution rule:

* if an exact same-name, same-kind type exists in the base BTF, classify the type as kernel-derived and add it to the seed set
* if no such match exists, classify the type as program-local and exclude it from the seed set without error

This is the only place where “no match” is not an error. Subsequent closure resolution and base-BTF matching treat unresolved already-classified kernel types as fatal.

Anonymous types and bitfield members must be preserved when reachable from a kernel-derived type, even though they cannot be matched by name.

This classification rule is intentionally simple and imperfect. It does not fully prevent collisions where a program-local type reuses the exact name and kind of a kernel type. v0.1 accepts this limitation.

Project convention for v0.1:

* program-local record types should avoid reusing kernel record names

Future versions may use richer provenance- or usage-aware classification, but v0.1 does not.

10.3 CO-RE-Based Seeds
...
The closure must support, at minimum:

* struct member types, named and anonymous
* union member types, named and anonymous
* typedef targets
...
* array element types
* enum definitions
* recursive pointer cycles that require forward declarations

v0.1 may conservatively over-include:

* pointer target expansions beyond the strict minimum
* function prototype-related dependent types
* bitfield-related dependent types
* flexible-array-adjacent dependent types
...
v0.1 assumes a single, self-contained base BTF used as the sole source of truth for kernel type resolution.

Cross-BTF reconciliation, split-BTF-aware matching, and flavored type handling are explicitly out of scope.

v0.1 uses one matching strategy only.

12.1 v0.1 Matching Strategy

v0.1 uses name + kind exact match.

For each required type extracted from object inputs, the tool searches the base BTF for a type with:

* identical name, string-equal and case-sensitive
* identical kind, such as STRUCT, UNION, ENUM, TYPEDEF

Resolution rules:

* if exactly one match exists, use it
* if zero matches exist during §10.2 Local-vs-Kernel Distinction, treat the type as program-local and exclude it
* if zero matches exist during §11 closure or §12 resolution of an already-classified kernel type, fail
* if multiple matches exist, fail
...
* fuzzy matching
* kind coercion, such as STRUCT ↔ FWD
* ___flavor suffix handling
...
14.1 Forward Declarations

When the type graph contains cycles, such as struct task_struct containing a pointer to struct task_struct, the emitter must produce forward declarations to break the cycle, following the convention used by bpftool btf dump file ... format c.
...
When available, error messages should include:

* the BPF program section or function name that triggered the requirement
* the source file and line number, if .BTF.ext line info is present

Example:

error: failed to resolve kernel type 'task_struct'
in:    foo.bpf.c:42 (function: handle_exec)
file:  foo.bpf.o
hint:  verify that --btf points to the intended target kernel BTF
...
* silently skip unresolved required types, except in §10.2 Local-vs-Kernel Distinction
...
Given identical inputs and tool version, the tool must produce identical output.

18.1 Determinism Conditions

Identical output requires:

* byte-identical base BTF
* byte-identical object inputs, or equivalently identical .BTF, .BTF.ext, and .symtab content
* identical tool version

The tool does not normalize or canonicalize input BTF.
...
* local-vs-kernel type distinction
...
7. typedef chain length greater than or equal to 3
8. anonymous struct nested inside named struct
9. union containing struct member
10. function pointer field in struct
11. recursive type via pointer, such as struct list_head
12. flexible array member
13. incomplete type used only as pointer, where forward declaration is sufficient
14. bitfield access via CO-RE relocation
15. program-local types coexisting with kernel-derived types in the same object
...
* fuzzy matching and kind coercion in §12
* weak type or optional CO-RE relocation handling beyond fail-fast
...
26. Relationship to Existing Tools

26.1 vs bpftool gen min_core_btf

min_core_btf produces a binary BTF file optimized for runtime distribution with shipped BPF objects. Its purpose is to let a precompiled .bpf.o run on kernels that lack BTF. It does not produce a C header and is not intended for compile-time use.

This tool is its compile-time counterpart: same general approach, object-driven type extraction, but different output, C header, and different audience, BPF source developers.

26.2 vs bpftool btf dump file ... format c [root_id ...]

btf dump format c with explicit root_ids produces a reduced C header, but requires the user to know which type IDs to specify. This is impractical as a daily workflow because:

* root_ids are numeric and change between BTF versions
* users typically know type names, not IDs
* CO-RE relocation roots are not directly visible to users

This tool computes the roots automatically from .bpf.o, then performs the equivalent of format c emission internally.

This tool is not a replacement for root_id-driven emission. It is an automation layer that derives those roots from object-local type usage.

26.3 vs hand-written slim vmlinux.h

Projects such as tracee and libbpf-tools maintain hand-written reduced vmlinux.h files. This is fragile: adding a new BPF program or changing a field access requires editing the header. This tool eliminates that maintenance.

27. Build Assumptions

The tool assumes input .bpf.o files are produced by a standard libbpf-style BPF build, specifically:

* compiled with clang -target bpf
* compiled with -g
* compiled with -O2

-g is required because BTF and .BTF.ext generation depends on debug information.

-O2 is assumed because v0.1 is defined against ordinary optimized libbpf-style builds, not debug-only -O0 builds.

If .bpf.o is built without -g, the object’s .BTF and .BTF.ext sections may be absent or incomplete, and the tool may fail or produce a header that is not compile-complete.

v0.1 does not attempt to detect or warn about every non-standard build flag combination. The user is responsible for ensuring inputs follow normal libbpf CO-RE build conventions.
