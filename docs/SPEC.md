...
Compile-complete also includes:
	•	emitted record types carry preserve_access_index either via per-type attribute or via #pragma clang attribute push/pop
...
Object-local BTF contains both kernel-derived types and program-local types (event structs, map value types, etc.). The tool must distinguish them by attempting to resolve each candidate type against the base BTF.
	•	If a same-name, same-kind type exists in the base BTF, treat it as a kernel type and add it to the seed set.
	•	If no match exists, treat it as a program-local type and exclude it from the seed set without error.
This is the only place where “no match” is not an error. Subsequent closure resolution (§11) and base-BTF matching (§12) treat unresolved types as fatal.
Anonymous types (struct/union without a name) and bitfield members must be preserved in the seed set when reachable from a kernel-derived type, even though they cannot be matched by name.
...
v0.1 uses name + kind exact match. For each required type extracted from object inputs, the tool searches the base BTF for a type with:
	•	identical name (string-equal, case-sensitive)
	•	identical kind (STRUCT, UNION, ENUM, TYPEDEF, etc.)
Resolution rules:
	...
	•	If zero matches exist during §11 closure or §12 resolution of an already-classified kernel type, fail (§17.3).
	•	If multiple matches exist, fail (§17.3).
v0.1 must not introduce:
	•	pluggable matchers
	•	dynamic rule selection
	•	policy configuration layers
	•	fuzzy matching
	•	kind coercion (e.g. STRUCT ↔ FWD)
...
When the type graph contains cycles (e.g. struct task_struct holds a struct task_struct *real_parent), the emitter must produce forward declarations to break the cycle, following the convention used by bpftool btf dump file ... format c.
...
When available, error messages should include:
	•	the BPF program section/function name that triggered the requirement
	•	the source file and line number, if .BTF.ext line_info is present
Example:
...
Identical output requires:
	...
	•	byte-identical object inputs (or, equivalently, identical .BTF, .BTF.ext, and .symtab section content)
...
Integration tests must cover at least:
	...
	7.	typedef chain length ≥ 3
	8.	anonymous struct nested inside named struct
	9.	union containing struct member
	10.	function pointer field in struct
	11.	recursive type via pointer (e.g. struct list_head)
	12.	flexible array member
	13.	incomplete type used only as pointer (forward declaration sufficient)
...
The following are intentionally deferred:
	...
	•	weak type / optional CO-RE relocation handling beyond fail-fast
...
This tool is its compile-time counterpart: same general approach (object-driven type extraction), different output (C header), different audience (BPF source developers).
...
26.3 vs hand-written slim vmlinux.h (e.g. tracee, libbpf-tools)
Projects like aquasecurity/tracee maintain hand-written reduced vmlinux.h files. This is fragile: adding a new BPF program or changing a field access requires editing the header. This tool eliminates that maintenance.
...
The tool assumes input .bpf.o files are produced by a standard libbpf-style BPF build, specifically:
	...
	•	compiled with -g (BTF and .BTF.ext generation requires DWARF or BTF debug info)
	•	compiled with -O2 (the standard optimization level for BPF programs)
If .bpf.o is built without -g, the object’s .BTF and .BTF.ext sections may be absent or incomplete, and the tool may fail or produce a header that is not compile-complete. v0.1 does not attempt to detect or warn about non-standard build flags; the user is responsible for ensuring inputs follow normal libbpf CO-RE build conventions.​​​​​​​​​​​​​​​​
