...
grep -q "relo: #1" "$tmpdir/stderr.txt"
grep -q "file: .*bad.bpf.o" "$tmpdir/stderr.txt"
grep -q "detail: CO-RE relocation object type:" "$tmpdir/stderr.txt"
grep -Eq "base BTF query: id [0-9]+ STRUCT 'definitely_not_a_kernel_type_for_min_corehdr' -> no same-name, same-kind match" \
  "$tmpdir/stderr.txt"
